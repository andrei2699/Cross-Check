...
class CsvFileComparatorTest {
    ...
    @ParameterizedTest
    @ValueSource(strings = {"file", "folder/file", "f.abc", "folder/file.abc", "file.csv.other", "folder/file.csv.other"})
    ...
    @ParameterizedTest
    @ValueSource(strings = {"file", "folder/file", "f.abc", "folder/file.abc", "file.csv.other", "folder/file.csv.other", "f.abc.csv", "folder/file.csv",})
    void canApply_withOverriddenExtensions__shouldReturnFalse_whenExtensionIsNotSupported(String path) {
    ...
    void compare_shouldReturnTheChangedValues_whenActualFileHasSameColumnsButDifferentValues() {
        Path expected = ResourceFileReader.getResourcePath("test-data/comparators/csv/input.csv");
        Path actual = ResourceFileReader.getResourcePath("test-data/comparators/csv/different-value.csv");
    ...
    void compare_shouldReturnTheChangedValues_whenActualFileHasDifferentColumnsButSameContent() {
        Path expected = ResourceFileReader.getResourcePath("test-data/comparators/csv/input.csv");
        Path actual = ResourceFileReader.getResourcePath("test-data/comparators/csv/different-column.csv");

        List<FileEntryDifference> differences = comparator.compare(expected, actual);

        Assertions.assertEquals(3, differences.size());
        Assertions.assertEquals(new FileEntryDifference(0, "string", "string1"), differences.getFirst());
    ...
    void compare_shouldReturnTheChangedValues_whenActualFileHasLessColumns() {
        Path expected = ResourceFileReader.getResourcePath("test-data/comparators/csv/input.csv");
        Path actual = ResourceFileReader.getResourcePath("test-data/comparators/csv/input-less-values.csv");
    ...
    void compare_shouldReturnTheChangedValues_whenActualFileHasMoreColumns() {
        Path expected = ResourceFileReader.getResourcePath("test-data/comparators/csv/input.csv");
        Path actual = ResourceFileReader.getResourcePath("test-data/comparators/csv/input-more-values.csv");
    ...
    void compare_withCustomCsvMapper_shouldReturnEmptyList_whenFilesDifferentHeaderCasing() {
        Path expected = ResourceFileReader.getResourcePath("test-data/comparators/csv/input.csv");
        Path actual = ResourceFileReader.getResourcePath("test-data/comparators/csv/input-with-comments.csv");
        comparator = new CsvFileComparator(List.of("csv"), CsvMapper.builder().enable(CsvReadFeature.ALLOW_COMMENTS).build());
