...
public final class CsvFileComparator implements FileComparator {
  private static final CsvMapper DEFAULT_CSV_MAPPER =
      CsvMapper.builder().enable(CsvReadFeature.WRAP_AS_ARRAY).build();
  ...
  public List<FileEntryDifference> compare(Path expected, Path actual) {
    ...
    try (MappingIterator<String[]> expectedIterator =
            mapper.readerFor(String[].class).readValues(expectedFile);
        MappingIterator<String[]> actualIterator =
            mapper.readerFor(String[].class).readValues(actualFile)) {
      long lineCount = 0;
  ...
  private List<FileEntryDifference> addRemainingExpectedLines(
      long lineNumber, MappingIterator<String[]> expectedIterator) {
    List<FileEntryDifference> result = new ArrayList<>();
  ...
  private List<FileEntryDifference> addRemainingActualLines(
      long lineNumber, MappingIterator<String[]> actualIterator) {
    List<FileEntryDifference> result = new ArrayList<>();
  ...
  private List<FileEntryDifference> addEntriesForLine(
      long lineNumber, String[] expectedRow, String[] actualRow) {
    List<FileEntryDifference> result = new ArrayList<>();
  ...
  private List<FileEntryDifference> addOnlyExpectedValues(
      long lineNumber, String[] expectedRow, int index) {
    List<FileEntryDifference> result = new ArrayList<>();
  ...
  private List<FileEntryDifference> addOnlyActualValues(
      long lineNumber, String[] actualRow, int index) {
    List<FileEntryDifference> result = new ArrayList<>();
